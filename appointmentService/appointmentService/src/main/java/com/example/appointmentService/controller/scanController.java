...
public class scanController {
    ...
    public ResponseEntity<List<scanResult>> getScanResultsForPatient(@PathVariable Long patientId) {
        ...
        // Check if any of the scan results do not match the patientId
        for (scanResult result : scanResults) {
            if (!result.getPatientId().equals(patientId)) {
                throw new ResponseStatusException(HttpStatus.FORBIDDEN, "You cannot view another patient's scan results");
            }
        }
